# FontGen3 v1.10 - フォント制作ツールキット

高品質なビットマップフォントの制作とTTF/BDF/WebFont形式での書き出しが可能な統合フォント制作環境です。

## 📦 構成

- **font_editor_main1.10.py** - メインフォントエディタ (270KB) ⭐️ 最新版
- **font_editor_main1.00.py** - メインフォントエディタ (225KB) - 安定版
...
```bash
python font_editor_main1.10.py  # 最新版（WebFont対応）
# または
python font_editor_main1.00.py  # 安定版
```
...
### フォントエディタ (font_editor_main1.10.py / 1.00.py)
...
- **アンドゥ/リドゥ**: 無制限の編集履歴
- **ガイドライン** 🆕: ベースライン、アセンダー、ディセンダー、中央線を表示

#### ✅ ファイル対応
- **読み込み**: TTF、OTF、TTC（複数フォント選択可能）、CSV
- **書き出し**:
  - TTF形式（高品質アウトライン変換、ハイブリッドマージ方式）
  - BDF形式（ビットマップフォント）
  - **WebFontパッケージ** 🆕: WOFF/SVG/TTF + HTMLサンプル自動生成
  - **CSV形式** 🆕: グリフデータの一括インポート/エクスポート
  - PNG一括書き出し
...
### v1.10 (2025-11-13) 🎉 NEW!
- ✅ **WebFont書き出し**: WOFF/SVG形式対応、HTMLサンプル自動生成
- ✅ **CSV I/O**: グリフデータの一括インポート/エクスポート
- ✅ **ガイドライン機能**: ベースライン、アセンダー、ディセンダー、中央線を表示
- ✅ TTEdit.exeの高度な機能を解析・参考に実装
...
### 今後の予定
- v1.20: IVS（異体字セレクタ）サポート、部首関係性管理
- v1.30: ノイズ除去フィルタ、メディアンフィルタ、太字/細字自動生成
- v2.00: Bスプライン曲線編集、ベクトル編集機能
