...
# ===== GlyphEditorクラスの続き =====
    ...
    def _update_preview(self) -> None:
        ...
        # ガイドライン描画 (2025-11-13: 表示フラグ対応)
        if self.show_guidelines.get():
            for guide_name, pos_y in self.guidelines:
                y_canvas = pos_y * self.zoom_level
                # 水平ガイドライン
                self.preview_canvas.create_line(0, y_canvas, new_width, y_canvas,
                                               fill='#00AAFF', dash=(5, 5), width=1, tags='guide')
                # ガイドライン名表示
                self.preview_canvas.create_text(5, y_canvas - 10, text=guide_name,
                                               fill='#00AAFF', anchor='w', font=('Arial', 9), tags='guide')
